...
----------------------------------------------------------------------------
--○ 쿼리문
-- ① TBL_DAILY (일일관리) 자동 등록 쿼리문
--1. DAILY_CODE 의 MAX 보다 +1 큰 코드 알아내기
SELECT CONCAT('DAILY', LPAD(NVL(SUBSTR(MAX(DAILY_CODE),6),0)+1, 3,0))
FROM TBL_DAILY;

--2. PET_CODE로 RELATION_CODE 찾기
SELECT RELATION_CODE
FROM TBL_RELATION
WHERE PET_CODE = 'PET003';

--3. TBL_DAILY에 INSERT 해보기 (완성)
...
--6. TBL_DAILY → 양육관계+해당날짜 데이터 있는지 COUNT
SELECT COUNT(*) AS COUNT
FROM TBL_DAILY
WHERE RELATION_CODE= 'REL003'
AND DAILY_DATE = SYSDATE;

--7. TBL_DAILY에 INSERT 하기
INSERT INTO TBL_DAILY(DAILY_CODE, RELATION_CODE, DAILY_DATE)
VALUES('DAILY11', 'REL003', SYSDATE);


--8. TBL_COMMON의 COMMON_CODE MAX값 
...
VALUES('COM27', TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS'), 'YYYY-MM-DD HH24:MI:SS')
...
--10. DAILY_CODE 찾기 (RELATION_CODE && DAILY_DATE 존재 시)
SELECT DAILY_CODE
FROM TBL_DAILY
WHERE RELATION_CODE = 'REL0003'
AND DAILY_DATE = SYSDATE;