...
-------------------------------------------------------------
-- ○ 반려견과 예방접종 뷰 
-- (TBL_RELATION & TBL_PET & TBL_PET_INFO & TBL_VACCINE & TBL_INJECT_TYPE)

CREATE OR REPLACE VIEW VACCINE_VIEW
AS
SELECT R.RELATION_CODE AS RELATIOIN_CODE
    , R.SID_CODE AS SID_CODE
    , R.PET_CODE AS PET_CODE
    , PI.PET_NAME AS PET_NAME
    , VA.INJECT_TYPE_CODE AS INJECT_TYPE_CODE
    , IJ.INJECT_TYPE_NAME AS INJECT_TYPE_NAME
FROM TBL_RELATION R
LEFT OUTER JOIN TBL_PET P ON R.PET_CODE = P.PET_CODE
LEFT OUTER JOIN TBL_PET_INFO PI ON P.PET_CODE = PI.PET_CODE
LEFT OUTER JOIN TBL_VACCINE VA ON R.PET_CODE = VA.PET_CODE
LEFT OUTER JOIN TBL_INJECT_TYPE IJ ON VA.INJECT_TYPE_CODE = IJ.INJECT_TYPE_CODE 
ORDER BY RELATION_CODE;


-- 다이어리 내 반려견 정보 → 예방접종내역
...
------------------------------------------------------------
--○ 기저질환 뷰 (TBL_PET & TBL_PET_INFO & TBL_DISEASE & TBL_DISEASE_TYPE)

CREATE OR REPLACE VIEW DISEASE_VIEW
AS
SELECT P.PET_CODE AS PET_CODE
    , PI.PET_NAME AS PET_NAME
    , D.DISEASE_CODE AS DISEASE_CODE
